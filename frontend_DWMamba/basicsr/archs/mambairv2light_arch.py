...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2., axes=None, learnable_fuse=False, axis_prompt_proj=False):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_tokens = num_tokens
        self.inner_rank = inner_rank
        # multi-axis config (Phase 1: support ['row','col'])
        if axes is not None and len(axes) < 2:
            axes = None
        if axes is not None:
            allowed = {'row','col','row_rev','col_rev'}
            sanitized = []
            for a in axes:
                if a not in allowed:
                    raise ValueError(f"Unsupported axis '{a}' (allowed: {allowed})")
                if a not in sanitized:
                    sanitized.append(a)
            axes = sanitized if len(sanitized) >= 2 else None
        self.axes = axes
        self.learnable_fuse = learnable_fuse and (self.axes is not None)
        if self.learnable_fuse:
            self.axis_weights = nn.Parameter(torch.zeros(len(self.axes)))  # softmax -> uniform start
        # Phase 3: per-axis prompt projection
        self.axis_prompt_proj_enabled = bool(axis_prompt_proj and self.axes is not None)
        if self.axis_prompt_proj_enabled:
            self.axis_prompt_proj = nn.ModuleDict({a: nn.Linear(d_state, d_state) for a in self.axes})
        else:
            self.axis_prompt_proj = None
    ...
    def forward(self, x, x_size, token):
        ...
        # Helper functions for axis reorder/restore (Phase 1: row/col)
        def axis_reorder(t, axis):
            if axis.startswith('row'):
                seq = t
            elif axis.startswith('col'):
                seq = t.view(B, H, W, -1).permute(0, 2, 1, 3).reshape(B, n, -1)
            else:
                raise ValueError(axis)
            if axis.endswith('rev'):
                seq = seq.flip(1)
            return seq

        def axis_restore(t, axis):
            if axis.endswith('rev'):
                t = t.flip(1)
            if axis.startswith('row'):
                return t
            elif axis.startswith('col'):
                return t.view(B, W, H, -1).permute(0, 2, 1, 3).reshape(B, n, -1)
            else:
                raise ValueError(axis)
        ...
        if self.axes is None:
            ...
            # multi-axis fusion (simple average Phase 1)
            outs = []
            for i, axis in enumerate(self.axes):
                seq = axis_reorder(semantic_x, axis)
                prompt_axis = axis_reorder(prompt, axis)  # keep alignment
                if self.axis_prompt_proj is not None:
                    # prompt_axis: (B, N, d_state) -> apply linear over last dim
                    prompt_axis = self.axis_prompt_proj[axis](prompt_axis)
                y_axis = self.selectiveScan(seq, prompt_axis)
                y_axis = self.out_proj(self.out_norm(y_axis))
                y_axis = axis_restore(y_axis, axis)
                y_axis = semantic_neighbor(y_axis, x_sort_indices_reverse)
                outs.append(y_axis)
            if self.learnable_fuse:
                w = torch.softmax(self.axis_weights, dim=0)  # (k,)
                fused = torch.stack(outs, dim=0)  # (k,B,N,C)
                x = torch.einsum('kbnc,k->bnc', fused, w)
            else:
                x = sum(outs) / len(outs)
            return x
...
class Selective_Scan(nn.Module):
    def __init__(
        ...
        # Prefer CUDA kernel; fallback to reference implementation on CPU.
        # (The CUDA op will raise if inputs are not on CUDA.)
    ...
    def forward_core(self, x: torch.Tensor, prompt):
        ...
        # Runtime fallback: if tensors on CPU but CUDA kernel chosen, switch to reference.
        selective_impl = self.selective_scan
        if not xs.is_cuda and selective_impl is selective_scan_fn:
            selective_impl = selective_scan_ref
        out_y = selective_impl(
            xs, dts,
            As, Bs, Cs, Ds, z=None,
...
class AttentiveLayer(nn.Module):
    def __init__(self,
        ...
        self.assm = ASSM(
            ...
            mlp_ratio=mlp_ratio,
            axes=axes,
            learnable_fuse=learnable_fuse,
            axis_prompt_proj=axis_prompt_proj,
        )
...
class MambaIRv2Light(nn.Module):
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 multi_axis=None,
                 multi_axis_fuse='avg',  # 'avg' | 'learned'
                 axis_prompt_proj=False,
                 entropy_lambda=0.0,
                 **kwargs):
        super().__init__()
        ...
        # store multi-axis config and entropy regularization weight
        ...
        self.layers = nn.ModuleList()
        learnable_fuse = (multi_axis_fuse == 'learned')
    ...
    # ----------------------------- Phase 4: Entropy Regularization Utilities ----------------------------- #
    def collect_axis_weights(self):
        """Return list of (module_ref, softmax_weights_tensor, axes_list)."""
        collected = []
        for layer in getattr(self, 'layers', []):
            # layer is ASSB
            if not hasattr(layer, 'residual_group'):
                continue
            for attn_layer in layer.residual_group.layers:  # each AttentiveLayer
                assm = getattr(attn_layer, 'assm', None)
                if assm is not None and getattr(assm, 'learnable_fuse', False):
                    w = torch.softmax(assm.axis_weights, dim=0)
                    collected.append((assm, w, assm.axes))
        return collected

    def axis_entropy_loss(self, reduction='mean'):
        """Compute Σ w log w over all ASSM with learnable_fuse (negative entropy); usually multiply by entropy_lambda.
        Args:
            reduction: 'mean'|'sum'|'none'
        Returns: scalar tensor (or tensor list if 'none').
        """
        items = []
        for _, w, _ in self.collect_axis_weights():
            items.append((w * (w.log() + 1e-8)).sum())  # ≤ 0
        if not items:
            return torch.zeros((), device=self.conv_first.weight.device)
        losses = torch.stack(items)
        if reduction == 'mean':
            return losses.mean()
        elif reduction == 'sum':
            return losses.sum()
    ...
    def extra_loss(self):
        """Return a dict of extra regularization losses (currently entropy). Training loop can add them.
        If entropy_lambda == 0 returns empty dict.
        """
        out = {}
        if self.entropy_lambda > 0:
            out['axis_entropy'] = self.entropy_lambda * self.axis_entropy_loss()
        return out

    def summarize_axis_fusion(self):
        """Return dict with mean fusion weights per axis name (for logging)."""
        stats = {}
        collected = self.collect_axis_weights()
        if not collected:
            return stats
        # aggregate by axis label
        tmp = {}
        counts = {}
        for _, w, axes in collected:
            for idx, ax in enumerate(axes):
                tmp.setdefault(ax, 0.0)
                counts.setdefault(ax, 0)
                tmp[ax] += w[idx].item()
                counts[ax] += 1
        for ax, v in tmp.items():
            stats[f'axis_w_{ax}'] = v / max(1, counts[ax])
        return stats
